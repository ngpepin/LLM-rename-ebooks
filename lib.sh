...
: "${ISBN_GREP_RF_SCAN_FIRST:=800}"
: "${ISBN_GREP_RF_REVERSE_LAST:=150}"
...
[ -z "${OUTPUT_FILENAME_TEMPLATE:+x}" ] && OUTPUT_FILENAME_TEMPLATE='"${d[TITLE]//:/ -}${d[SERIES]:+ [${d[SERIES]//:/ -}]} - ${d[AUTHORS]// & /, }${d[PUBLISHED]:+ (${d[PUBLISHED]%%-*})}${d[ISBN]:+ [${d[ISBN]}]}.${d[EXT]}"'
...
move_failed_file() {
	local _file_path="$1"
	local new_path="$(unique_filename "${OUTPUT_FOLDER_FAILED%/}" "$(basename "$_file_path")")"
	move_or_link_file "$_file_path" "$new_path"
}
