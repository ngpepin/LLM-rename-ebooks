...
for arg in "$@"; do
	...
	-off=* | --output-folder-failed=*) OUTPUT_FOLDER_FAILED="${arg#*=}" ;;
...
for fpath in "$@"; do
	...
	find "$fpath" -type f -print0 | sort -z ${FILE_SORT_FLAGS[@]:+"${FILE_SORT_FLAGS[@]}"} | while IFS= read -r -d '' file_to_check; do
		# Use a subshell to isolate errors for each file
		(
			# Exit the subshell on error
			set -e

			# Call the function and handle errors
			if ! organize_file "$file_to_check" 2> >(debug_prefixer "[$file_to_check] " "$DEBUG_PREFIX_LENGTH"); then
				decho "Error processing file: $file_to_check"
			fi
		)

		# Continue to the next file even if the subshell fails
		true
	done
done
